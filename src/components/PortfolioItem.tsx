...
export const PortfolioItem = ({ title, description, image, technologies, complexity }: PortfolioItemProps) => {
  ...
  <div className="aspect-video bg-gradient-to-br from-primary/20 to-blue-600/20 flex items-center justify-center relative overflow-hidden">
    <div className="text-2xl sm:text-3xl text-primary/60 font-bold group-hover:scale-110 transition-transform duration-300">
      MT4/5
    </div>
  ...
  <div className="p-4 sm:p-6">
    <div className="flex items-center justify-between mb-1 sm:mb-2">
      ...
      <Badge
        ...
        className="group-hover:scale-110 transition-transform duration-300 text-[10px] sm:text-xs"
      >
        {complexity}
      </Badge>
    </div>
    <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4 group-hover:text-foreground transition-colors duration-300">
      {description}
    </p>
    <div className="flex flex-wrap gap-1 sm:gap-2">
      {technologies.map((tech, index) => (
        <Badge
          key={tech}
          variant="outline"
          className="text-[10px] sm:text-xs hover:bg-primary hover:text-primary-foreground transition-colors duration-300 animate-fade-in"
